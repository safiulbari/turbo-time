...
export default function PomodoroTimer({
  ...
  return <div className="w-full">
      {/* Task Header */}
      {currentTask && <div className="mb-4">
      ...
      <div className="timer-card p-4">
        {/* Session Tabs */}
        <div className="flex gap-1 mb-4 p-0.5 rounded-lg bg-secondary/20">
          <button onClick={() => handleTabChange("work")} disabled={isActive} className={`tab-button flex-1 flex items-center justify-center gap-1.5 py-2 text-xs rounded-md transition-all ${selectedTab === "work" ? "tab-active" : "tab-inactive"} ${isActive ? "opacity-50 cursor-not-allowed" : ""}`}>
            <Zap className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Focus</span>
            <span className="text-xs opacity-70">25m</span>
          </button>

          <button onClick={() => handleTabChange("break")} disabled={isActive} className={`tab-button flex-1 flex items-center justify-center gap-1.5 py-2 text-xs rounded-md transition-all ${selectedTab === "break" ? "tab-active" : "tab-inactive"} ${isActive ? "opacity-50 cursor-not-allowed" : ""}`}>
        ...
        <div className="relative mb-4 flex justify-center">
          <CircularProgress progress={progress} sessionType={sessionType} size={160} />
        ...
        <div className="flex gap-2 justify-center mb-3">
          <Button onClick={handleStartPause} className="btn-primary px-5" size="sm">
            {isActive ? <>
          ...
          <Button onClick={handleReset} className="btn-icon" size="sm">
            <RotateCcw className="w-4 h-4" />
          </Button>
        </div>

        {/* Stats */}
        {sessionCount > 0 && <div className="flex items-center justify-center gap-1.5 pt-3 border-t border-border/20">
            <Trophy className="w-3.5 h-3.5 text-amber-500" />
            <span className="text-xs text-muted-foreground">
              <span className="font-semibold text-foreground">{sessionCount}</span> completed
            </span>
          </div>}
      </div>
    </div>;
